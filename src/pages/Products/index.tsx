...
function Products() {
  ...
  async function loadProducts() {
    ...
    try {
      ...
      const result = await productService.getAll(selectedCategory, isGlobal);
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
            ...
            <label className="flex items-center gap-2 cursor-pointer bg-white hover:bg-gray-50 px-3 py-2 rounded border border-gray-300 shadow-sm transition-colors">
                ...
                />
                <span className="text-sm font-medium text-gray-700">Ver Todas as Lojas</span>
            </label>
      ...
      {loading ? (
        <div className="p-10 text-center">
            ...
            <p className="mt-2 text-rose-600 font-medium">Carregando produtos...</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm bg-white">
                 ...
                 <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                   <td className="px-6 py-2">
                     <img 
                       ...
                       onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/50x50?text=Erro'; }}
                     />
                   </td>
                   <td className="px-6 py-4 font-mono text-xs text-gray-600">{product.code}</td>
                   <td className="px-6 py-4 font-medium text-gray-900">{product.name}</td>
                   <td className="px-6 py-4 text-gray-600">{product.category_name || '-'}</td>
                   <td className="px-6 py-4 text-rose-600 font-bold">{formatMoney(product.rental_price)}</td>
          ...
          {!loading && products.length === 0 && (
            <div className="p-10 text-center text-gray-500 bg-white">
                ...
                {isGlobal === false && (
                    <p className="text-xs mt-2 text-gray-400">Tente marcar "Ver Todas as Lojas".</p>
                )}
            </div>
          )}
        </div>
